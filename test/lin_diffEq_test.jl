...
res = solve_QuDiff(QuEuler,A, b, x, tspan, h)
...
res = solve_QuDiff(QuLeapfrog,A, b, x, tspan, h)
r = res[Int((N_t+1)*2+2^N -1): Int((N_t+1)*2+2^N + N_t - 3)] # range of relevant values in the obtained state.
@test isapprox.(s,r,atol = 0.3) |> all

res = solve_QuDiff(QuAB2,A, b, x, tspan, h)
r = res[Int((N_t+1)*2+2^N -1): Int((N_t+1)*2+2^N + N_t - 3)] # range of relevant values in the obtained state.
@test isapprox.(s,r,atol = 0.3) |> all
...
res = solve_QuDiff(QuAB3,A, b, x, tspan, h)
r = res[Int((N_t+1)*2+2^N -1): Int((N_t+1)*2+2^N + N_t - 3)] # range of relevant values in the obtained state.
@test isapprox.(s,r,atol = 0.3) |> all

res = solve_QuDiff(QuAB4,A, b, x, tspan, h)
r = res[Int((N_t+1)*2+2^N -1): Int((N_t+1)*2+2^N + N_t - 3)] # range of relevant values in the obtained state.
@test isapprox.(s,r,atol = 0.3) |> all
